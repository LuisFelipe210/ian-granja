...
const Atuacao = () => {
    ...
    return (
        <div className="min-h-screen bg-background font-sans selection:bg-accent selection:text-accent-foreground">
            ...
            {/* --- HERO SECTION: Sóbria mas Suavizada --- */}
            ...
            {/* --- ÁREAS DE ATUAÇÃO: CARDS FLUTUANTES --- */}
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {practiceAreas.map((area, index) => (
                            ...
                            >
                                {/* Efeito de Glow (Brilho) atrás do card no Hover */}
                                ...
                                {/* O Card em si */}
                                <div className="relative h-full bg-white border border-border/60 hover:border-border p-8 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col">

                                    {/* Ícone com fundo estiloso */}
                                ...
                                {howWeWork.map((item, index) => (
                                    <div key={index} className="flex gap-6 group">
                                        <div className="flex flex-col items-center">
                                            <div className="w-12 h-12 rounded-full border-2 border-accent/30 bg-background flex items-center justify-center text-accent font-bold text-lg group-hover:border-accent group-hover:bg-accent group-hover:text-accent-foreground transition-all duration-300 shrink-0 shadow-md">
                                                {item.step}
                                            </div>
                                            {index !== howWeWork.length - 1 && (
                                                <div className="w-px h-full bg-border my-2 group-hover:bg-accent/30 transition-colors min-h-[40px]"></div>
                                            )}
                                        </div>
            ...
            <section className="py-24 bg-muted/20">
                <div className="container mx-auto px-4 text-center">
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
                        ...
                        <Button variant="outline" size="xl" className="h-14 px-8 text-lg border-2 bg-white rounded-full hover:bg-primary min-w-[240px] hover:border-accent hover:text-accent transition-all" asChild>
                            <NavLink to="/sobre">
